...
bool flagPenis = true; // отвечает за то будем ли мы брать время начала нажатия (чтобы брать только один раз)
bool flagVagina = false;
...
void workButton(){
  if (!flagPenis && millis() - timeStartPress >= consts.HOLD_TIME * 2){
    countButtonClick = 1;
    flagVagina = false;
    flagPenis = true;
    flagReadyWork = true;
